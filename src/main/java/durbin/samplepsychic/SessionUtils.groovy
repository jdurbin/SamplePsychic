...
class SessionUtils {
	...
	//	String jobOutputPath = getSubdirPath("jobs/")
		def fileName = "${jobDir}/samplePsychic_${token}.results"
	...
	static def generateResultsTokenAndFileName(jobDir){		
		...
		while(!ready){
			...
			fileName = getResultsFileNameFromToken(token,jobDir);
	 ...
	 * @return
	 */
	static String getResultsFileNameFromToken0(token){
		// e.g. subdir = "/jobs"	
		System.err.println "SessionUtils.getResultsFileNameFromToken token:"+token.class
		String jobOutputPath = getSubdirPath("jobs/")
	...
	static def generateResultsTokenAndFileName0(jobDir){		
		// Probability of collision is very low, but non-zero, so 
		// build in something to try multiple times. 
		def ready = false;
		def token
